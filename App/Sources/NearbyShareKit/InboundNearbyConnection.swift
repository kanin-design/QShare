...
class InboundNearbyConnection: NearbyConnection{
	...
	// QuickShare2 addition: aggregate byte counters so we can report overall
	// incoming-transfer progress in-app (upstream NearDrop only publishes a
	// system-level NSProgress per file).
	private var totalBytesToReceive:Int64=0
	private var totalBytesReceived:Int64=0

	// QuickShare2 addition: where completed files landed, so the app can
	// open/reveal them.
	public private(set) var savedFileURLs:[URL]=[]

	private func reportProgress(){
		guard totalBytesToReceive>0 else { return }
		let fraction=min(1.0, Double(totalBytesReceived)/Double(totalBytesToReceive))
		DispatchQueue.main.async {
			self.delegate?.connection(self, transferProgress: fraction)
		}
	}
	...
	override func processFileChunk(frame: Location_Nearby_Connections_PayloadTransferFrame) throws{
		...
		if frame.payloadChunk.body.count>0{
			...
			totalBytesReceived+=Int64(frame.payloadChunk.body.count)   // QuickShare2
			reportProgress()                                            // QuickShare2
			...
			savedFileURLs.append(fileInfo.destinationURL)   // QuickShare2
	...
	private func processIntroductionFrame(_ frame:Sharing_Nearby_Frame) throws{
		...
		if frame.v1.introduction.fileMetadata.count>0 && frame.v1.introduction.textMetadata.isEmpty{
			...
			for file in frame.v1.introduction.fileMetadata{
				...
				// destination to the Downloads directory as defense in depth.
				let safeName=InboundNearbyConnection.sanitizeFileName(file.name)
				let dest=makeFileDestinationURL(downloadsDirectory.appendingPathComponent(safeName))
				guard dest.resolvingSymlinksInPath().path.hasPrefix(downloadsDirectory.path+"/") else {
					throw NearbyError.protocolError("Rejected file with unsafe name")
				}
	...
	private func acceptTransfer(){
		do{
			totalBytesToReceive=transferredFiles.values.reduce(Int64(0)){ $0 + $1.meta.size }   // QuickShare2
			totalBytesReceived=0                                                                 // QuickShare2
...
protocol InboundNearbyConnectionDelegate{
	...
	// QuickShare2 addition — optional overall progress for the incoming transfer.
	func connection(_ connection:InboundNearbyConnection, transferProgress:Double)
}

// Default no-op so existing conformers need not implement the progress hook.
extension InboundNearbyConnectionDelegate{
	func connection(_ connection:InboundNearbyConnection, transferProgress:Double){}
}
