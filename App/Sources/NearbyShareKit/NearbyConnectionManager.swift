...
public protocol MainAppDelegate{
	...
	// QuickShare2 addition — overall progress for an accepted incoming transfer.
...
public class NearbyConnectionManager : NSObject, NetServiceDelegate, InboundNearbyConnectionDelegate, OutboundNearbyConnectionDelegate{
	...
	func connectionWasTerminated(connection:InboundNearbyConnection, error:Error?){
		guard let delegate=mainAppDelegate else {return}
		if error==nil, !connection.savedFileURLs.isEmpty {   // QuickShare2
			delegate.incomingTransfer(id: connection.id, savedFiles: connection.savedFileURLs)
		}
		delegate.incomingTransfer(id: connection.id, didFinishWith: error)
		activeConnections.removeValue(forKey: connection.id)
	}

	// QuickShare2 addition — relay incoming progress to the app.
	func connection(_ connection:InboundNearbyConnection, transferProgress:Double){
		mainAppDelegate?.incomingTransfer(id: connection.id, progress: transferProgress)
	}
